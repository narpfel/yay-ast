 ...
 * an abstract syntax tree (AST). The main function is YayAST_FromNode().
 *
 */
#include "Python.h"
...
mod_ty
YayAST_FromNodeObject(const node *n, PyCompilerFlags *flags,
                     PyObject *filename, PyArena *arena)
{
    ...
    switch (TYPE(n)) {
        case file_input:
            stmts = _Yay_asdl_seq_new(num_stmts(n), arena);
            if (!stmts)
                goto out;
            for (i = 0; i < NCH(n) - 1; i++) {
                ch = CHILD(n, i);
        ...
        case single_input:
            if (TYPE(CHILD(n, 0)) == NEWLINE) {
                stmts = _Yay_asdl_seq_new(1, arena);
                if (!stmts)
                    goto out;
                ...
                n = CHILD(n, 0);
                num = num_stmts(n);
                stmts = _Yay_asdl_seq_new(num, arena);
                if (!stmts)
                    goto out;
                if (num == 1) {
                    s = ast_for_stmt(&c, n);
        ...
        default:
            PyErr_Format(PyExc_SystemError,
                         "invalid node %d for YayAST_FromNode", TYPE(n));
            goto out;
    }
 out:
    if (c.c_normalize) {
...
mod_ty
YayAST_FromNode(const node *n, PyCompilerFlags *flags, const char *filename_str,
               PyArena *arena)
{
    mod_ty mod;
    PyObject *filename;
...
seq_for_testlist(struct compiling *c, const node *n)
{
    ...
    */
    ...
    seq = _Yay_asdl_seq_new((NCH(n) + 1) / 2, c->c_arena);
    if (!seq)
        return NULL;
...
ast_for_arguments(struct compiling *c, const node *n)
{
    ...
    */
    ...
    posargs = (nposargs ? _Yay_asdl_seq_new(nposargs, c->c_arena) : NULL);
    if (!posargs && nposargs)
        return NULL;
    kwonlyargs = (nkwonlyargs ?
                   _Yay_asdl_seq_new(nkwonlyargs, c->c_arena) : NULL);
    if (!kwonlyargs && nkwonlyargs)
        return NULL;
    posdefaults = (nposdefaults ?
                    _Yay_asdl_seq_new(nposdefaults, c->c_arena) : NULL);
...
ast_for_decorators(struct compiling *c, const node *n)
{
    ...
    REQ(n, decorators);
    decorator_seq = _Yay_asdl_seq_new(NCH(n), c->c_arena);
    if (!decorator_seq)
        return NULL;
...
ast_for_comprehension(struct compiling *c, const node *n)
{
    ...
    comps = _Yay_asdl_seq_new(n_fors, c->c_arena);
    if (!comps)
        return NULL;
    ...
    for (i = 0; i < n_fors; i++) {
        ...
        if (NCH(n) == (5 + is_async)) {
            ...
            ifs = _Yay_asdl_seq_new(n_ifs, c->c_arena);
            if (!ifs)
                return NULL;
...
ast_for_dictdisplay(struct compiling *c, const node *n)
{
    ...
    keys = _Yay_asdl_seq_new(size, c->c_arena);
    if (!keys)
        return NULL;

    values = _Yay_asdl_seq_new(size, c->c_arena);
    if (!values)
        return NULL;
...
ast_for_setdisplay(struct compiling *c, const node *n)
{
    ...
    elts = _Yay_asdl_seq_new(size, c->c_arena);
    if (!elts)
        return NULL;
    for (i = 0; i < NCH(n); i += 2) {
        expr_ty expression;
    ...
    if (TYPE(CHILD(n, 0)) == LPAR) {
        ...
        if (NCH(n) == 1) {
            ...
            */
            ...
            slices = _Yay_asdl_seq_new((NCH(n) + 1) / 2, c->c_arena);
            if (!slices)
                return NULL;
            for (j = 0; j < NCH(n); j += 2) {
            ...
            elts = _Yay_asdl_seq_new(asdl_seq_LEN(slices), c->c_arena);
            if (!elts)
                return NULL;
    ...
    switch (TYPE(n)) {
        ...
        case comparison:
            if (NCH(n) == 1) {
                ...
                asdl_int_seq *ops;
                asdl_seq *cmps;
                ops = _Yay_asdl_int_seq_new(NCH(n) / 2, c->c_arena);
                if (!ops)
                    return NULL;
                cmps = _Yay_asdl_seq_new(NCH(n) / 2, c->c_arena);
                if (!cmps) {
                    return NULL;
                }
                for (i = 1; i < NCH(n); i += 2) {
...
ast_for_call(struct compiling *c, const node *n, expr_ty func)
{
    ...
    */
    ...
    args = _Yay_asdl_seq_new(nargs + ngens, c->c_arena);
    if (!args)
        return NULL;
    keywords = _Yay_asdl_seq_new(nkeywords, c->c_arena);
    if (!keywords)
        return NULL;
...
ast_for_import_stmt(struct compiling *c, const node *n)
{
    ...
    */
    ...
    if (TYPE(n) == import_name) {
        ...
        aliases = _Yay_asdl_seq_new((NCH(n) + 1) / 2, c->c_arena);
        if (!aliases)
                return NULL;
        for (i = 0; i < NCH(n); i += 2) {
       ...
       /* Count the number of dots (for relative imports) and check for the
        ...
        aliases = _Yay_asdl_seq_new((n_children + 1) / 2, c->c_arena);
        if (!aliases)
            return NULL;
...
ast_for_global_stmt(struct compiling *c, const node *n)
{
    ...
    REQ(n, global_stmt);
    s = _Yay_asdl_seq_new(NCH(n) / 2, c->c_arena);
    if (!s)
        return NULL;
    for (i = 1; i < NCH(n); i += 2) {
...
ast_for_suite(struct compiling *c, const node *n)
{
    ...
    total = num_stmts(n);
    seq = _Yay_asdl_seq_new(total, c->c_arena);
    if (!seq)
        return NULL;
    ...
    */
    if (s[2] == 's') {
        ...
        if (has_else) {
            ...
            orelse = _Yay_asdl_seq_new(1, c->c_arena);
            if (!orelse)
                return NULL;
        ...
        for (i = 0; i < n_elif; i++) {
            ...
            asdl_seq *newobj = _Yay_asdl_seq_new(1, c->c_arena);
            if (!newobj)
                return NULL;
...
ast_for_try_stmt(struct compiling *c, const node *n)
{
    ...
    if (n_except > 0) {
        ...
        handlers = _Yay_asdl_seq_new(n_except, c->c_arena);
        if (handlers == NULL)
            return NULL;
...
ast_for_with_stmt(struct compiling *c, const node *n, int is_async)
{
    ...
    n_items = (NCH(n) - 2) / 2;
    items = _Yay_asdl_seq_new(n_items, c->c_arena);
    if (!items)
        return NULL;
...
ExprList_Finish(ExprList *l, PyArena *arena)
{
    ...
    seq = _Yay_asdl_seq_new(l->size, arena);
    if (seq) {
        Py_ssize_t i;
        for (i = 0; i < l->size; i++)

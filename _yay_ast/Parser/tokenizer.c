...
const char *_YayParser_TokenNames[] = {
    "ENDMARKER",
    "NAME",
    "NUMBER",
    "STRING",
...
error_ret(struct tok_state *tok) /* XXX */
{
    ...
    if (tok->fp != NULL && tok->buf != NULL) /* see YayTokenizer_Free */
        PyMem_FREE(tok->buf);
    tok->buf = tok->cur = tok->end = tok->inp = tok->start = NULL;
    tok->done = E_DECODE;
...
struct tok_state *
YayTokenizer_FromString(const char *str, int exec_input)
{
...
struct tok_state *
YayTokenizer_FromUTF8(const char *str, int exec_input)
{
    struct tok_state *tok = tok_new();
    if (tok == NULL)
        return NULL;
...
struct tok_state *
YayTokenizer_FromFile(FILE *fp, const char* enc,
...
void
YayTokenizer_Free(struct tok_state *tok)
{
    if (tok->encoding != NULL)
        PyMem_FREE(tok->encoding);
#ifndef PGEN
...
int
YayToken_TwoChars(int c1, int c2)
{
    switch (c1) {
    case '=':
        switch (c2) {
...
int
YayToken_ThreeChars(int c1, int c2, int c3)
{
    switch (c1) {
    case '<':
        switch (c2) {
 ...
 again:
  ...
  letter_quote:
    ...
    /* Check for two-character token */
    {
        ...
        int token = YayToken_TwoChars(c, c2);
        if (token != OP) {
            int c3 = tok_nextc(tok);
            int token3 = YayToken_ThreeChars(c, c2, c3);
            if (token3 != OP) {
                token = token3;
            }
            else {
    ...
    *p_start = tok->start;
    *p_end = tok->cur;
    return YayToken_OneChar(c);
}

int
YayTokenizer_Get(struct tok_state *tok, char **p_start, char **p_end)
{
...
/* Get the encoding of a Python file. Check for the coding cookie and check if
   ...
   YayTokenizer_FindEncodingFilename() returns NULL when it can't find the
...
char *
YayTokenizer_FindEncodingFilename(int fd, PyObject *filename)
{
    struct tok_state *tok;
    FILE *fp;
    ...
    tok = YayTokenizer_FromFile(fp, NULL, NULL, NULL);
    if (tok == NULL) {
        fclose(fp);
        return NULL;
    }
    ...
    while (tok->lineno < 2 && tok->done == E_OK) {
        YayTokenizer_Get(tok, &p_start, &p_end);
    }
    fclose(fp);
    if (tok->encoding) {
...
char *
YayTokenizer_FindEncoding(int fd)
{
    return YayTokenizer_FindEncodingFilename(fd, NULL);
}
...
{
    printf("%s", _YayParser_TokenNames[type]);

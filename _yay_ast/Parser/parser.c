...
parser_state *
YayParser_New(grammar *g, int start)
{
    parser_state *ps;

    if (!g->g_accel)
        YayGrammar_AddAccelerators(g);
    ...
    ps->p_flags = 0;
#endif
    ps->p_tree = YayNode_New(start);
    if (ps->p_tree == NULL) {
        PyMem_FREE(ps);
        return NULL;
    }
    s_reset(&ps->p_stack);
    (void) s_push(&ps->p_stack, YayGrammar_FindDFA(g, start), ps->p_tree);
    return ps;
}

void
YayParser_Delete(parser_state *ps)
{
    /* NB If you want to save the parse tree,
       you must set p_tree to NULL before calling delparser! */
    YayNode_Free(ps->p_tree);
    PyMem_FREE(ps);
}
...
shift(stack *s, int type, char *str, int newstate, int lineno, int col_offset)
{
    int err;
    assert(!s_empty(s));
    err = YayNode_AddChild(s->s_top->s_parent, type, str, lineno, col_offset);
    if (err)
        return err;
...
push(stack *s, int type, dfa *d, int newstate, int lineno, int col_offset)
{
    ...
    n = s->s_top->s_parent;
    assert(!s_empty(s));
    err = YayNode_AddChild(n, type, (char *)NULL, lineno, col_offset);
    if (err)
        return err;
    s->s_top->s_state = newstate;
...
int
YayParser_AddToken(parser_state *ps, int type, char *str,
                  int lineno, int col_offset, int *expected_ret)
{
    int ilabel;
    int err;

    D(printf("Token %s/'%s' ... ", _YayParser_TokenNames[type], str));
...
dumptree(grammar *g, node *n)
{
    ...
    if (n == NULL)
        ...
        l.lb_type = TYPE(n);
        l.lb_str = STR(n);
        printf("%s", YayGrammar_LabelRepr(&l));
...
showtree(grammar *g, node *n)
{
    ...
    if (ISNONTERMINAL(TYPE(n))) {
        ...
        printf("%s", _YayParser_TokenNames[TYPE(n)]);
